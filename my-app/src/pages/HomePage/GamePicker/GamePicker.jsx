import GameFeaturePicker from '../GameFeaturePicker/GameFeaturePicker.jsx';
import styles from './GamePicker.module.css';
import { useEffect, useState } from 'react';
import image from '@/assets/images/game_picker_game_photo.png';
import { useDispatch, useSelector } from 'react-redux';
import { selectCategories } from '@/redux/selectors.js';
...
export default function GamePicker() {
    ...
    const gettingProductStatus = useSelector(
        ({ carouselProducts: { statusOfProduct } }) => statusOfProduct
    );
    ...
    const pickGame = async (e) => {
        ...
        const filters = {
            ...
            playersAmount: playersAmount && [
                getMinMaxObjFromStr(playersAmount),
            ],
        };
        const {
            payload: { product },
        } = await dispatch(getRandomProduct(filters));
        if (product) {
        ...
        <section className={styles.wrap}>
            ...
            {noMatchingProduct && (
                <PopUp
                    ...
                    title={'Гру за запитом не знайдено :('}
